...
Every image is provided as `image.jpg`, as MusiCorpus requires. These JPEGs are compressed hard — OLiMPiC publishes PNG, and a faithful JPEG re-encoding of near-bilevel scanned paper is several times the size of the PNG it came from. Expect visible ringing around staff lines and note heads at 1:1 zoom; every symbol remains unambiguous, and for training data this behaves as a mild augmentation.
...
`splits.json` carries the OLiMPiC splits, with `dev` renamed to `validation` as MusiCorpus names it. The scanned variant is distributed as a dev and a test set only, so the `train` split is present but empty; the synthetic variant is where the training data is.
