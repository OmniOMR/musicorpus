...
def distribute_page_metadata(
    ...
    for page_name in tqdm.tqdm(page_names, "Distributing page metadata"):
        ...
        if page_name not in page_metadatas:
            errors.add_error(page_name, "Page metadata missing in the input metadata CSV.")
            continue

        # write the file
        page_metadatas[page_name].write_to_file(output_folder / page_name / "metadata.json")
