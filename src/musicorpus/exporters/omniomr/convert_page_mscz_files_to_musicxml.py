...
def convert_page_mscz_files_to_musicxml(
    page_names: list[str], output_folder: Path, errors: ErrorBag
):
    """
    ...
    for page_name in page_names:
        ...
        if not source_path.exists():
            errors.add_error(
                page_name,
                "Source mscz file for conversion to musicxml missing at: " + str(source_path),
            )
            continue
    ...
    # run MuseScore
    musescore_batch_convert(conversion_map=conversion_map, force_replace_existing_files=True)
