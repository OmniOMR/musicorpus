...
def create_subdivisions_folders(page_names: list[str], output_folder: Path, errors: ErrorBag):
    ...
    for page_name in tqdm.tqdm(page_names, "Creating subdivisions folders"):
        ...
        if not source_path.exists():
            errors.add_error(page_name, "subdivisions.image.json not found in: " + str(source_path))
            continue
