...
def mung2mxl_staff_index(mung_staff_number: int, empty_staves: list[int]) -> int:
    ...
    empty_staves_above = len(
        [staff_number for staff_number in empty_staves if staff_number <= mung_staff_number]
    )
...
def subdivide_musicxml_files(
    ...
    for page_name in tqdm.tqdm(page_names, "Subdividing musicxml files"):
        ...
        if page_name not in layout_file.records:
            errors.add_error(page_name, "Layout record missing for the page.")
            continue
        layout_record = layout_file.records[page_name]

        # load page musicxml
        musicxml_tree: ET.ElementTree = ET.ElementTree(
            ET.fromstring((output_folder / page_name / "transcription.musicxml").read_text("utf-8"))
        )
        ...
        if layout_map.page_count != 1:
            errors.add_error(page_name, "MusicXML files must be single-page")
            continue

        if (
            layout_map.staff_count_on_page(0) + len(layout_record.empty_staves)
            != layout_record.staff_count
        ):
            errors.add_error(page_name, "MusicXML does not have matching staff count with MuNG")
            continue
        ...
        for staff_name in subdivisions.staves:
            ...
            try:
                write_musicxml_tree_to_file(
                    (output_folder / page_name / "Staves" / staff_name / "transcription.musicxml"),
                    extract_staff(
        ...
        for page_system_index, system_name in enumerate(subdivisions.systems.keys()):
            try:
                write_musicxml_tree_to_file(
                    ...
                    extract_system(
                        layout_map=layout_map, page_index=0, page_system_index=page_system_index
                    ),
                )
            except Exception:
                errors.add_error(
