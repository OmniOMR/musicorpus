...
class ImageSubdivisions:
    ...
    def load_from(file_path: Path) -> "ImageSubdivisions":
        ...
        return ImageSubdivisions(
            staves={name: CocoBbox.from_json(d.get("bbox")) for name, d in staves.items()},
            grandstaves={
                name: CocoBbox.from_json(d.get("bbox")) for name, d in grandstaves.items()
            },
            systems={name: CocoBbox.from_json(d.get("bbox")) for name, d in systems.items()},
        )

    def write_to(self, file_path: Path):
        """Writes the data to a 'subdivisions.image.json' file."""
        data = {
            "Staves": {name: {"bbox": list(bbox)} for name, bbox in self.staves.items()},
            "Grandstaves": {name: {"bbox": list(bbox)} for name, bbox in self.grandstaves.items()},
            "Systems": {name: {"bbox": list(bbox)} for name, bbox in self.systems.items()},
        }
