...
def validate_coco_object_detection_file(
    dataset_path: Path, coco_file: Path, manifest: MusicorpusManifest, errors: ErrorBag
):
    ...
    categories: dict[int, str] = validate_categories(
        page_name=page_name, coco_file=coco_file, data_categories=data_categories, errors=errors
    )
...
def validate_info_block(
    page_name: str, coco_file: Path, data_info: dict, manifest: MusicorpusManifest, errors: ErrorBag
):
...
def validate_licenses(
    dataset_path: Path, page_name: str, coco_file: Path, data_licenses: list[dict], errors: ErrorBag
) -> set[int]:
    license_ids = set()
...
def validate_images(
    ...
    for i, data_image in enumerate(data_images):
        ...
        if "id" not in data_image:
            errors.add_error(
                page_name=page_name,
                message=f"The COCO file's 'images[{i}].id' " + f"field is missing. In: {coco_file}",
            )
            continue
...
def validate_categories(
    page_name: str, coco_file: Path, data_categories: list[dict], errors: ErrorBag
):
    ...
    for i, data_annotation in enumerate(data_annotations):
        ...
        try:
            ...
            if type(data_segmentation) in [list, dict]:
                frPyObjects(pyobj=data_segmentation, h=data_bbox.height, w=data_bbox.width)
            else:
                raise ValueError(
