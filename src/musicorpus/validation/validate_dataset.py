...
def validate_dataset(dataset_path: Path, errors: ErrorBag):
    page_names_by_folders: list[str] = [f.name for f in dataset_path.iterdir() if f.is_dir()]
    ...
    if not (dataset_path / "musicorpus.json").exists():
        errors.add_error(
            page_name="root",
            message="The musicorpus.json file " + "must be present in the dataset root.",
        )
        return  # no more checking can be done without the manifest file
    manifest = MusicorpusManifest.load_from_file(dataset_path / "musicorpus.json")
    ...
    expected_dataset_folder_name = (
        manifest.short_institution_name + "." + manifest.short_dataset_name
    )
    ...
    if not (dataset_path / "README.md").exists():
        errors.add_error(
            page_name="root", message="The README.md file " + "must be present in the dataset root."
        )
    ...
    if not (dataset_path / "LICENSE.txt").exists():
        errors.add_error(
            page_name="root",
            message="The LICENSE.txt file " + "must be present in the dataset root.",
        )

    # splits.json and splits.*.json
    splits_files = [dataset_path / "splits.json"] + list(dataset_path.glob("splits.*.json"))
    for splits_file_path in splits_files:
        ...
        if not splits_file_path.exists():
            ...
            try:
                splits = Splits.read_from_file(file_path=splits_file_path, run_assertions=True)
                splits.check_that_it_covers_page_names_exactly(
                    page_names_by_folders, raise_on_failure=True
                )
            except Exception:
                errors.add_error(
                    page_name="root",
    ...
    page_files: set[str] = {
        file.name for page in page_folders for file in page.iterdir() if file.is_file()
    }
    staff_files: set[str] = {
        file.name for staff in staff_folders for file in staff.iterdir() if file.is_file()
    }
    grandstaff_files: set[str] = {
        file.name
        for grandstaff in grandstaff_folders
        for file in grandstaff.iterdir()
        if file.is_file()
    }
    system_files: set[str] = {
        file.name for system in system_folders for file in system.iterdir() if file.is_file()
    }
    ...
    for metadata_file in tqdm.tqdm(
        list(dataset_path.glob("**/metadata.json")), "Validating metadata.json files"
    ):
        validate_metadata_file(
            dataset_path=dataset_path, metadata_file=metadata_file, errors=errors
        )
    ...
    for coco_file in tqdm.tqdm(
        ...
        validate_coco_object_detection_file(
            dataset_path=dataset_path, coco_file=coco_file, manifest=manifest, errors=errors
        )

    # layout.json
    for layout_file in tqdm.tqdm(
        list(dataset_path.glob("**/layout.json")), "Validating layout.json files"
    ):
        validate_layout_file(
            dataset_path=dataset_path, layout_file=layout_file, manifest=manifest, errors=errors
        )
    ...
    for musicxml_file in tqdm.tqdm(
        ...
        validate_musicxml_file(
            dataset_path=dataset_path, musicxml_file=musicxml_file, errors=errors
        )

    # transcription.mung
    for mung_file in tqdm.tqdm(
        list(dataset_path.glob("**/transcription.mung")), "Validating transcription.mung files"
    ):
        validate_mung_file(dataset_path=dataset_path, mung_file=mung_file, errors=errors)
