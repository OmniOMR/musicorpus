...
5. Update `DOWNLOAD_URL` in `src/musicorpus/exporters/download_specification_pdf.py` and `SPECIFICATION_VERSION` in `src/musicorpus/__init__.py`, then release the package too — a package that implements a specification version should say which one.
